  ...
  try {
    ...
    if (!activeSnap.empty || !activeSnap2.empty) {
      return res.status(400).json({
        error: 'You\'re already paired with someone! Check your active Hawt Dates below to answer their questions.',
        alreadyActive: true,
      });
    }

    // Get recent partners (last 7 days) to avoid repeat pairings
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const sevenDaysAgoTimestamp = admin.firestore.Timestamp.fromDate(sevenDaysAgo);

    const recentSnap1 = await db.collection('hawtDates')
      .where('user1Id', '==', userId)
      .where('createdAt', '>=', sevenDaysAgoTimestamp)
      .get();
    const recentSnap2 = await db.collection('hawtDates')
      .where('user2Id', '==', userId)
      .where('createdAt', '>=', sevenDaysAgoTimestamp)
      .get();

    const recentPartnerIds = new Set();
    recentSnap1.forEach(doc => recentPartnerIds.add(doc.data().user2Id));
    recentSnap2.forEach(doc => recentPartnerIds.add(doc.data().user1Id));
    ...
    candidatesSnap.forEach(doc => {
      ...
      // Skip recent partners (cooldown period)
      if (recentPartnerIds.has(doc.id)) return;
    ...
    if (candidates.length === 0) {
      // Check if it's because everyone available was a recent partner
      const allCandidatesCount = [];
      candidatesSnap.forEach(doc => {
        if (doc.id !== userId) allCandidatesCount.push(doc.id);
      });
      const allRecentPartners = allCandidatesCount.length > 0 && allCandidatesCount.every(id => recentPartnerIds.has(id));

      return res.status(404).json({
        error: allRecentPartners
          ? 'You\'ve already Hawt Dated everyone available recently. Check back in a few days for new matches!'
          : 'No available partners found right now. Check back soon!',
        noPartner: true,
      });
    }
...
router.post('/:hawtDateId/like', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Mark this Hawt Date as completed for this user so it clears from their active list
    const finishedField = isUser1 ? 'user1ReviewDone' : 'user2ReviewDone';
    const otherFinishedField = isUser1 ? 'user2ReviewDone' : 'user1ReviewDone';
    const updates = { [finishedField]: true };
    if (hawtDate[otherFinishedField]) updates.status = 'completed';
    await hawtDateRef.update(updates);
...
// POST /api/hawtdate/:hawtDateId/pass
// Pass on the other user after a no-match reveal (or skip accepting a Hawt Match)
router.post('/:hawtDateId/pass', verifyToken, async (req, res) => {
  const userId = req.user.uid;
  const { hawtDateId } = req.params;
  ...
  try {
    ...
    // Mark this Hawt Date as reviewed/done for this user so it clears from their active list
    const finishedField = isUser1 ? 'user1ReviewDone' : 'user2ReviewDone';
    const otherFinishedField = isUser1 ? 'user2ReviewDone' : 'user1ReviewDone';
    const updates = { [finishedField]: true };
    if (hawtDate[otherFinishedField]) updates.status = 'completed';
    ...
    console.error('Hawt date pass error:', error);
    return res.status(500).json({ error: 'Server error' });
  }
});
...
router.get('/active', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const addDate = async (doc) => {
      ...
      // Skip dates this user has already reviewed (liked/passed) on a no-match reveal,
      // unless it's now a confirmed mutual match (status 'matched') worth showing.
      const isUser1 = data.user1Id === userId;
      const myReviewDone = isUser1 ? data.user1ReviewDone : data.user2ReviewDone;
      if (data.status === 'revealed' && !data.isHawtMatch && myReviewDone) return;
