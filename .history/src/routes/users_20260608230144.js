...
router.get('/nearby', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Get current user's coordinates as fallback for distance display
    ...
    // Geocode the filter city (Google handles both city names and zip codes)
    let filterCoords = null;
    if (city && city.trim()) {
      filterCoords = await getCoordsFromCity(city);
      console.log(`[users/nearby] city="${city}" filterCoords=`, filterCoords);
    }

    // Distance display origin: use filter city if searching, otherwise user's own location
    const distanceOrigin = filterCoords || currentUserCoords;
    ...
    snapshot.forEach(docSnap => {
      ...
      // Filter by radius from searched city
      if (filterCoords && data.coordinates) {
        const distanceFromFilter = getDistanceMiles(
          filterCoords.lat, filterCoords.lng,
          data.coordinates.lat, data.coordinates.lng
        );
        if (distanceFromFilter > radiusMiles) return;
      } else if (city && city.trim()) {
        // Fallback: text match if geocoding failed
        const userCity = (data.city || '').toLowerCase();
        const filterCity = city.toLowerCase().split(',')[0].trim();
        if (!userCity.includes(filterCity)) return;
      }

      // Distance shown on cards — from searched city if active, else from user's profile
      let displayDistance = null;
      if (distanceOrigin && data.coordinates) {
        displayDistance = getDistanceMiles(
          distanceOrigin.lat, distanceOrigin.lng,
          data.coordinates.lat, data.coordinates.lng
        );
      }
      ...
      if (data.boostExpiresAt) {
        const expiresAt = data.boostExpiresAt._seconds
          ? new Date(data.boostExpiresAt._seconds * 1000)
          : new Date(data.boostExpiresAt);
      ...
      results.push({
        ...
        distance: displayDistance !== null ? Math.round(displayDistance) : null,
        isBoosted,
      });
    });

    // Boosted users first, then by distance from search origin
