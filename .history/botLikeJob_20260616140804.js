...
async function createMatch(uid1, uid2) {
  ...
  batch.set(db.collection("users").doc(uid1).collection("matches").doc(uid2), {
    matchedAt: admin.firestore.FieldValue.serverTimestamp(),
    fromBot: true,
  });
  batch.set(db.collection("users").doc(uid2).collection("matches").doc(uid1), {
    matchedAt: admin.firestore.FieldValue.serverTimestamp(),
    fromBot: true,
  });
  await batch.commit();
...
async function runBotLikeJob() {
  ...
  console.log(
    `Found ${botsSnap.size} bots and ${realUsers.length} real users\n`,
  );
  ...
  for (const botDoc of botsSnap.docs) {
    ...
    console.log(
      `Processing ${bot.name} (${bot.botAutoLike ? "auto-like" : "like-back"})...`,
    );

    // Get who this bot has already swiped on
    const existingSwipesSnap = await db
      .collection("users")
      .doc(botUid)
      .collection("swipes")
      .get();
