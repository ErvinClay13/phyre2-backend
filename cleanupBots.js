...
async function cleanupBots() {
  ...
  for (const botDoc of botsSnap.docs) {
    ...
    try {
      ...
      await deleteCollection(
        db.collection("users").doc(botUid).collection("swipes"),
      );
      await deleteCollection(
        db.collection("users").doc(botUid).collection("likes"),
      );
      await deleteCollection(
        db.collection("users").doc(botUid).collection("likedBy"),
      );
      await deleteCollection(
        db.collection("users").doc(botUid).collection("matches"),
      );
      await deleteCollection(
        db.collection("users").doc(botUid).collection("blocked"),
      );
      await deleteCollection(
        db.collection("users").doc(botUid).collection("blockedBy"),
      );
      ...
      console.error(
        `     ❌ Failed to fully remove ${bot.name}:`,
        error.message,
      );
    }
  }
  ...
  console.log(
    "   All bot profiles, matches, and conversations have been deleted.\n",
  );
  process.exit(0);
}
